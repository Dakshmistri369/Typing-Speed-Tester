...
inputEl.addEventListener('keydown', e => {
  ...
  if (e.key === ' ') {
    e.preventDefault();
    if (state.wordTyped.length === 0) return;
    if (state.settings.sound) playSpace();
    handleSpace();
    return;
  }

  // Printable chars only (desktop physical keyboard)
  if (e.key.length !== 1) return;
  if (!isMobile()) {
    // On desktop we handle chars here; on mobile, `input` event handles it
    if (!state.started) startTimer();
    handleChar(e.key);
    // Prevent the char from appearing in the input field
    e.preventDefault();
  }
});

// Mobile: track previous value to detect what was typed/deleted
let prevInputVal = '';
...
inputEl.addEventListener('input', () => {
  ...
  if (!isMobile()) {
    // Desktop: always clear — keydown handles everything
    inputEl.value = '';
    prevInputVal = '';
    return;
  }
  ...
  // ── MOBILE PATH ──
  // Detect if a character was added or removed
  if (cur.length > prevInputVal.length) {
    // Characters were added
    const added = cur.slice(prevInputVal.length);
    if (!state.started) startTimer();

    // Check for space in added text
    for (const ch of added) {
      if (ch === ' ') {
        if (state.wordTyped.length > 0) {
          if (state.settings.sound) playSpace();
          handleSpace();
        }
      } else {
        handleChar(ch);
        if (state.settings.sound) playKey(true);
      }
    }
  } else if (cur.length < prevInputVal.length) {
    // Characters were deleted (backspace on virtual keyboard)
    handleBackspace();
  }
...
/* ===== TABS (desktop + mobile nav) ===== */
function switchTab(tabName) {
  document.querySelectorAll('.nav-btn, .mobile-nav-btn').forEach(b => b.classList.remove('active'));
  document.querySelectorAll('.tab-content').forEach(t => t.classList.remove('active'));
  document.querySelectorAll(`.nav-btn[data-tab="${tabName}"], .mobile-nav-btn[data-tab="${tabName}"]`).forEach(b => b.classList.add('active'));
  $('tab-' + tabName).classList.add('active');
  if (tabName === 'results') updateHistory();
}
...
document.querySelectorAll('[data-time]').forEach(btn => {
  btn.addEventListener('click', () => { document.querySelectorAll('[data-time]').forEach(b => b.classList.remove('active')); btn.classList.add('active'); state.timeLimit = +btn.dataset.time; reset(); if (!isMobile()) inputEl.focus(); });
});
document.querySelectorAll('[data-mode]').forEach(btn => {
  btn.addEventListener('click', () => { document.querySelectorAll('[data-mode]').forEach(b => b.classList.remove('active')); btn.classList.add('active'); state.mode = btn.dataset.mode; reset(); if (!isMobile()) inputEl.focus(); });
});
document.querySelectorAll('[data-diff]').forEach(btn => {
  btn.addEventListener('click', () => { document.querySelectorAll('[data-diff]').forEach(b => b.classList.remove('active')); btn.classList.add('active'); state.difficulty = btn.dataset.diff; reset(); if (!isMobile()) inputEl.focus(); });
});
...
function reset() {
  ...
  statusText.textContent = isMobile() ? 'Tap the button below to start' : 'Click here or press any key to start';
...
$('mobile-keyboard-btn').addEventListener('click', () => {
  inputEl.focus();
  // Update status hint
  statusText.textContent = 'Start typing…';
});

// Wire up the tap-to-type: clicking the words display on mobile also focuses
...
$('modal-restart').addEventListener('click', () => { $('result-modal').classList.remove('open'); reset(); if (!isMobile()) inputEl.focus(); });
$('modal-history').addEventListener('click', () => { $('result-modal').classList.remove('open'); switchTab('results'); });
...
/* ===== CONTROLS ===== */
$('refresh-btn').addEventListener('click', () => { reset(); if (!isMobile()) inputEl.focus(); });
$('restart-btn').addEventListener('click', () => { reset(); if (!isMobile()) inputEl.focus(); });
...
reset();
updateHistory();
// Only auto-focus input on desktop (mobile: wait for user tap)
